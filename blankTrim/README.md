...
三步流水线，最终只有中文逗号（，）和句号（。）保留给听书软件断句：

1. **标点 → 逗号**：中英文标点（～~！!？?、…—：:；;．. 引号 括号 书名号等）替换为中文逗号，连续多个只出一个
2. **广告符号 → 删除**：无意义符号（- = + * # @ _ | 等）直接删掉
3. **空白 → 删除**：所有空白字符删掉（ASCII 空白 + Unicode 全角空格/不间断空格/零宽空格等）

修改规则：[MainActivity.kt](app/src/main/java/com/example/blanktrim/MainActivity.kt) 里 `PUNCTUATION_TO_COMMA`、`JUNK_SYMBOLS_TO_REMOVE`、`WHITESPACE_PATTERN` 三个正则，直接改 `[...]` 里的字符即可。
